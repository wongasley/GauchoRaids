...
function ISSafehouseUI:initialise()
    ...
    -- Prevent safehouse menu data population when raiding
    if GauchoRaidClientState.isRaiding then return end
...
-- prevent adding raiders to the player list
                local isRaider = false
                for k,raiderName in pairs(GauchoRaidClientState.raidedBy) do
                    if raiderName == newPlayer.name then
                        isRaider = true
                    end
                end
...
function ISSafehouseUI:render()
    -- Prevent safehouse data polulation when raiding
    if not GauchoRaidClientState.isRaiding then
        self:updateButtons();
...
--    self:drawText(self.safehouse:getTitle(), self.width/2 - (getTextManager():MeasureStringX(UIFont.Medium, self.safehouse:getTitle()) / 2), z, 1,1,1,1, UIFont.Medium);
    ...
    -- Prevent safehouse data population when raiding
    if not GauchoRaidClientState.isRaiding then
...
function ISSafehouseUI:onQuitSafehouse(button)
    if button.internal == "YES" then
        if not getServerOptions():getBoolean("SafehouseAllowTrepass") then
            print("self > ", self)
            print("ui > ", button.parent.ui)
            print("CURRENT SAFEHOUSE > ", button.parent.ui.safehouse)
            print("REMOVING PLAYER > ", button.parent.ui.player:getUsername())
            local safehouse = button.parent.ui.safehouse
            local playerName = button.parent.ui.player:getUsername()
            sendClientCommand("GauchoRaids", "DefaultRemoveFromSafehouse", { safehouse:getX(), safehouse:getY(), playerName});
        end
...
function ISSafehouseUI:onRemovePlayerFromSafehouse(button, player)
    if button.internal == "YES" then
        if not getServerOptions():getBoolean("SafehouseAllowTrepass") then
            ...
            print("ui > ", button.parent.ui)
            print("CURRENT SAFEHOUSE > ", button.parent.ui.safehouse)
            print("REMOVING PLAYER > ", button.parent.ui.selectedPlayer)
            local safehouse = button.parent.ui.safehouse
            local playerName = button.parent.ui.selectedPlayer
            sendClientCommand("GauchoRaids", "DefaultRemoveFromSafehouse", { safehouse:getX(), safehouse:getY(), playerName});
        end
...
function ISSafehouseUI:new(x, y, width, height, safehouse, player)
    ...
    -- Prevent safehouse population when raiding
    if not GauchoRaidClientState.isRaiding then
...
-- Joaquin D. Gomez: Removed validation in case user is already in a safehouse 
